...
const GestionReservaTrabajadores = () => {
    ...
    useEffect(() => {
        ...
        firebase.buscarDocumentoPorCampo('estacionamientos', 'idEstacionamiento', userLocalStorage.idEstacionamiento)
            .then((dataEstacionamientos) => {
                if (dataEstacionamientos === null) {
                    alert('Debe crear primero o pertenecer a un estacionamiento');
                    navigate('/dashboard');
                    return;
                }
                ...
                if (dataEstacionamientos[0] === undefined) {
                    ...
                    buscarReservas(dataEstacionamientos.idEstacionamiento);
                } else {
                    setEstacionamientoSeleccionado(dataEstacionamientos[0]);
                    setDatos((datos) => ({ ...datos, idEstacionamiento: dataEstacionamientos.idEstacionamiento, nombreEstacionamiento: dataEstacionamientos.nombreEstacionamiento, }));
                    buscarReservas(dataEstacionamientos[0].idEstacionamiento);
                }
    ...
    const buscarReservas = (idEstacionamiento) => {
        const firebase = new Firebase();
        firebase.obtenerValorPorUnCampoEspecifico('reservas', 'estacionamiento.idEstacionamiento', idEstacionamiento)
            .then((reservasDb) => {
    ...
    const handleChange = (event, index) => {
        ...
        // Copia de los estacionamientos
        const estacionamientosCopia = [...estacionamientos];

        // Copia del trabajador actual
        const trabajadorActualizado = { ...estacionamientosCopia[index] };

        trabajadorActualizado[name] = value;

        // Actualizar la copia de estacionamientos con el trabajador actualizado
        estacionamientosCopia[index] = trabajadorActualizado;

        // Actualizar el estado con los estacionamientos actualizados
        setEstacionamientos(estacionamientosCopia);
    };

    return (
        <div className='container-dashboard row h-100'>
            <div className="table-responsive my-3">
                <div className="d-flex justify-content-center align-items-center m-3 mb-3">
...
export default GestionReservaTrabajadores